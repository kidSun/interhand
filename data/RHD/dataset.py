...
from utils.auc_evaluate import auc_validate
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inputs = {'img': img}
        targets = {'joint_coord': joint_coord, 'rel_root_depth': rel_root_depth, 'hand_type': hand_type, 'joint_cam':joint_cam}
        meta_info = {'joint_valid': joint_valid, 'root_valid': root_valid, 'inv_trans': inv_trans, 'hand_type_valid': 1,'cam_param':data['cam_param'] }
    ...
    def evaluate(self, preds):
        ...
        mpjpe_recon = [[] for _ in range(self.joint_num)]
        mpjpe_process = [[] for _ in range(self.joint_num)]
        acc_hand_cls = 0
        ...
        for n in range(sample_num):
            ...
            inv_map = [0,4,3,2,1,8,7,6,5,12,11,10,9,16,15,14,13,20,19,18,17]

            recon_coord_cam = pred_joint_coord_cam.copy()
            recon_coord_cam = recon_coord_cam[self.joint_type[gt_hand_type]][map]
            recon_coord_gt = gt_joint_coord.copy()
            recon_coord_gt = recon_coord_gt[self.joint_type[gt_hand_type]][map]
            joint_valid = joint_valid[self.joint_type[gt_hand_type]]
            if gt_hand_type=='right':
                j3d_recon, j3d_gt_process, j3d_pre_process = reconstruct(recon_coord_cam, recon_coord_gt,False, 'right',0)
            else:
                j3d_recon, j3d_gt_process, j3d_pre_process = reconstruct(recon_coord_cam, recon_coord_gt,False, 'left',0)
            

            auc_validate(j3d_pre_process, j3d_gt_process,"RHD",evaluator)

            j3d_recon = j3d_recon - j3d_recon[0]
            j3d_gt_process = j3d_gt_process - j3d_gt_process[0]
            j3d_pre_process = j3d_pre_process - j3d_pre_process[0]

            j3d_recon = j3d_recon[inv_map]
            j3d_gt_process = j3d_gt_process[inv_map]
            j3d_pre_process = j3d_pre_process[inv_map]
            ...
            for j in range(self.joint_num):
                if joint_valid[j]:
                    ...
                    mpjpe_recon[j].append(np.sqrt(np.sum((j3d_recon[j] - j3d_gt_process[j])**2)))
                    mpjpe_process[j].append(np.sqrt(np.sum((j3d_pre_process[j] - j3d_gt_process[j])**2)))
        ...
        (
            epe_mean_all, _2, epe_median_all,
            auc_all,
            pck_curve_all,
            thresholds
        ...
        print("EPE_mean of {}_test_set is : {}".format("RHD",epe_mean_all))
        print("EPE_median of {}_test_set is : {}".format("RHD",epe_median_all))
        print("AUC all of {}_test_set is : {}".format("RHD", auc_all))
        ...
        for j in range(self.joint_num):
            mpjpe_recon[j] = np.mean(np.stack(mpjpe_recon[j]))
            joint_name = self.skeleton[j]['name']
            eval_summary += (joint_name + ': %.2f, ' % mpjpe_recon[j])
        print(eval_summary)
        print('recon MPJPE: %.2f' % (np.mean(mpjpe_recon)))

        eval_summary = 'process MPJPE for each joint: \n'
        for j in range(self.joint_num):
            mpjpe_process[j] = np.mean(np.stack(mpjpe_process[j]))
            joint_name = self.skeleton[j]['name']
            eval_summary += (joint_name + ': %.2f, ' % mpjpe_process[j])
        print(eval_summary)
        print('process MPJPE: %.2f' % (np.mean(mpjpe_process)))