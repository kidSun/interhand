...
class MANOShapeLoss(nn.Module):
    ...
    def forward(self, shape_vector, shape_vector_gt, shape_valid):
        reg = 1e-6
        loss = (shape_vector - shape_vector_gt)**2 * shape_valid
        loss.mean()
        loss = loss + reg * 0.5*torch.sum(torch.pow(shape_vector, 2))
        return loss
